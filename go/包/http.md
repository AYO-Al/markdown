...
```
### 2.2.2 FormValue(key string) string

- 作用：获取 URL 查询参数或 POST 表单中指定键的值（自动调用 `ParseForm`）。
- 注意：若键存在多个值，返回第一个值。
### 2.2.3 PostFormValue(key string) string

- 作用：仅获取 POST 表单中指定键的值（自动调用 `ParseForm`）。
### 2.2.4 ParseMultipartForm(maxMemory int64) error
...
```
### 2.2.5 Cookie(name string) (\*Cookie, error)

- 作用：获取指定名称的 Cookie。
### 2.2.6 WithContext(ctx context.Context) \*Request
...
### 2.4.1 NewServeMux() \*ServeMux

- 作用：创建一个新的ServeMux对象
### 2.4.2 Handle(pattern string, handler http.Handler)
...
```
### 2.4.3 HandleFunc(pattern string, handler func(http.ResponseWriter, \*http.Request))
...
```
### 2.4.4 Handler(r \*http.Request) (handler http.Handler, pattern string)
...
```
### 2.4.5 ServeHTTP(w http.ResponseWriter, r \*http.Request)
