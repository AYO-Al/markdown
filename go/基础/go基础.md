...
![](../../.gitbook/assets/go基础_time_1.png)

此外，还有大约30多个预定义的名字，比如int和true等，主要对应内建的常量、类型和函数。

![](../../.gitbook/assets/go基础_time_2.png)
...
详细的如何写注释可以 参考：<[http://golang.org/doc/effective\_go.html#commentary](http://golang.org/doc/effective_go.html#commentary)>
...
1.  ​**​声明时无类型​**​\
    ...
    const num = 42 // 无类型整数常量 const str = "hello" // 无类型字符串常量 const ratio = 3.14 // 无类型浮点常量 const enabled = true // 无类型布尔常量
2.  ​**​自动类型推导​**​\
    当赋值给变量或参与运算时，自动转换为合适类型：

    var a int = num // 被用作int var b float64 = ratio // 被用作float64
...
| 序号 | 类型和描述                                                                                                                                                                                         |
| -- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1  | <p><strong>布尔型</strong><br>布尔型的值只可以是常量 true 或者 false。一个简单的例子：var b bool = true。</p>                                                                                                           |
| 2  | <p><strong>数字类型</strong><br>整型 int 和浮点型 float32、float64，Go 语言支持整型和浮点型数字，并且支持复数，其中位的运算采用补码。</p>                                                                                                |
| 3  | <p><strong>字符串类型:</strong><br>字符串就是一串固定长度的字符连接起来的字符序列。Go 的字符串是由单个字节连接起来的。Go 语言的字符串的字节使用 UTF-8 编码标识 Unicode 文本。<strong>请注意单引号：字符，双引号：字符串和``：原始字符串字面量</strong></p>                              |
| 4  | <p><strong>派生类型:</strong><br>包括：<br><br>- (a) 指针类型（Pointer）<br>- (b) 数组类型<br>- (c) 结构化类型(struct)<br>- (d) Channel 类型<br>- (e) 函数类型<br>- (f) 切片类型<br>- (g) 接口类型（interface）<br>- (h) Map 类型</p> |
...
| 序号 | 类型和描述                                                                                    |
| -- | ---------------------------------------------------------------------------------------- |
| 1  | <p><strong>uint8</strong><br>无符号 8 位整型 (0 到 255)</p>                                     |
| 2  | <p><strong>uint16</strong><br>无符号 16 位整型 (0 到 65535)</p>                                 |
| 3  | <p><strong>uint32</strong><br>无符号 32 位整型 (0 到 4294967295)</p>                            |
| 4  | <p><strong>uint64</strong><br>无符号 64 位整型 (0 到 18446744073709551615)</p>                  |
| 5  | <p><strong>int8</strong><br>有符号 8 位整型 (-128 到 127)</p>                                   |
| 6  | <p><strong>int16</strong><br>有符号 16 位整型 (-32768 到 32767)</p>                             |
| 7  | <p><strong>int32</strong><br>有符号 32 位整型 (-2147483648 到 2147483647)</p>                   |
| 8  | <p><strong>int64</strong><br>有符号 64 位整型 (-9223372036854775808 到 9223372036854775807)</p> |

#### 8.1.1 浮点型

| 序号 | 类型和描述                                               |
| -- | --------------------------------------------------- |
| 1  | <p><strong>float32</strong><br>IEEE-754 32位浮点型数</p> |
| 2  | <p><strong>float64</strong><br>IEEE-754 64位浮点型数</p> |
| 3  | <p><strong>complex64</strong><br>32 位实数和虚数</p>      |
| 4  | <p><strong>complex128</strong><br>64 位实数和虚数</p>     |
...
| 序号 | 类型和描述                                             |
| -- | ------------------------------------------------- |
| 1  | <p><strong>byte</strong><br>类似 uint8</p>          |
| 2  | <p><strong>rune</strong><br>类似 int32</p>          |
| 3  | <p><strong>uint</strong><br>32 或 64 位</p>         |
| 4  | <p><strong>int</strong><br>与 uint 一样大小</p>        |
| 5  | <p><strong>uintptr</strong><br>无符号整型，用于存放一个指针</p> |
...
![](https://www.runoob.com/wp-content/uploads/2015/06/ZBuVRsKmCoH6fzoz.png)
...
* 浅拷贝：只复制对象本身，而不复制对象引用的其他对象。如果对象中包含指针或引用类型，浅拷贝会复制这些指针的值（即内存地址），因此新对象和原对象会共享这些引用指向的数据。修改其中一个对象可能会影响另一个对象。
* 深拷贝：递归地复制对象本身以及对象引用的所有对象，直到所有层级都被复制。这样新对象和原对象完全独立，修改其中一个不会影响另一个。
...
Hash，又称散列，把任意长度的输入通过算法转换为固定长度的输出。
...
* x的范围是输入空间，可以是任意长度
* y的范围是输出空间，一般是固定长度
* 由于输入空间比输出空间大的多，所以可能出现不同的x计算得到相同的y，也称为碰撞或冲突
* 不同的x计算得到的y应该在输出空间均匀分布，减少碰撞
* y不能逆推回x，hash算法不可逆
* x一个微小的变化，哪怕是一个bit的变化，也将引起结果y的巨大变化
...
// 清除：slice元素清零，map长度清零
// clear 1.21+ 只作用于map和slice
clear(m)
```
...
delete() 函数用于删除集合的元素, 参数为 map 和其对应的 key，不存在不会报异常。实例如下：
...
```go
// 大致结构
// go/go1.24.0/src/runtime/map_noswiss.go
type hmap struct {
count // 元素个数
B  // 2^B桶数组长度 
buckets // 桶数组头地址
...
* **桶定位操作**：hash(key) % 2 ^ B = BucketIndex
* **C**：BucketIndex查是否有相同的key，没有则创建
* **U**：BucketIndex查是否有相同的key，有则更新
* **R**：BucketIndex遍历查key
* **D**：BucketIndex遍历查key，找到就删除

> **map结构**

底层结构为一个桶数组，每个桶数组位存放一个链表，每个链表的node为一个长度为8的数组。

> **哈希冲突**

如果哈希表底层数据结构为一个数组的话，但输入key过多，就有可能发生哈希冲突，也就是不同的key哈希过后的值可能一样，这样存放位置就冲突了，一般来说解决哈希冲突有以下几种方法：

1. 开放地址法：当不同的key哈希过后位置一样时，把位置往后挪，直到可以存放该key
2. 拉链法：不直接在数组中存放数据，而是在每个数组中存放一个链表，冲突也没关系，直接添加到链表中即可，go采用的就是这种方法

> **扩容**

因为Go采用的是拉链法，当一个数组位中的链表过长时会极大影响查找效率，所以当哈希表的负载因子 **LoadFactor = count / 2 ^ B > 6.5** 时，触发扩容机制。count大于8时启用负载因子。

Go中的扩容策略是 **B++**，也就是翻倍扩容，这样能保证一部分数据不需要重新迁移。当发生扩容时，Go会创建一个新的桶数组(这时候新老桶数组并存)，新写入的数据都写到新桶数组中，查的时候先查新桶数组，再查老桶数组。

Go采用的是渐进式迁移，也就是只有在有key写入的时候才从老桶数组中随机最多拿两个桶迁移到新桶数组中，避免一次性数据大量迁移影响性能。

有两种扩容方式：

* 增量扩容：就是上面所说的
* 等量扩容：map的链接node非常特殊，是一个长度为8的数组，且删除数组中的元素不会导致数据移动。所以当一个map元素数量不多，但node数组中由于删除元素导致过多的空洞导致node过多时，就会触发增量扩容，链表重排减少空洞
  * 即使负载因子不高，如果 `map`中​**​溢出桶的数量过多​**​，也会触发扩容。这通常发生在频繁地删除和插入键值对的场景中
  * 具体的判定规则是：
    * 当 `B`(桶数量的对数，桶数 = 2^B) ​**​小于 15​**​ 时，如果溢出桶数量 ​**​超过 2^B​**​，就会触发。
    * 当 `B`​**​大于等于 15​**​ 时，如果溢出桶数量 ​**​超过 2^15​**​，就会触发。
    * 这种情况下会触发 ​**​等量扩容​**​（也称为“整理扩容”）
  * 等量扩容并不会增加桶的数量，而是会分配一个同样大小的新桶数组，然后将旧的键值对重新哈希并“紧凑”地排列到新桶中，​**​目的是回收那些因频繁删除而产生的空位，减少溢出桶的数量​**​，从而提升后续操作的性能
...
如果使用 `[]int{1,2,3}...` 方式传递，则相当于header复制。位置传递则会创建底层数组，所以不允许混用。
...
**自由变量**：未在本地作用域定义的变量。例如在内层函数外的外层函数作用域中定义的变量。 **闭包**：出现在嵌套函数中，指的是内层函数引用到了外层函数的自由变量，就形成了闭包。闭包中使用的变量会逃逸到堆上。

可以使用 `go build -gcflags="-m -m" main.go` 查看逃逸信息。
...
方法名可以相同，只要调用者不同即可。recevier可以只是一个类型，也就是标注为该类型的方法，但不调用该类型的值。
...
函数定义在代码编译时会变成指令存放到代码段中，当调用函数时，编译器会创建一个相关函数的栈帧存放临时变量

整个栈帧会存放到函数调用栈中，每次调用函数结束后这个栈帧会消亡，如果其他函数中调用了这个函数并接收了返回值，会在消亡前复制到调用函数栈帧中。

![](../../.gitbook/assets/go基础_time_3.png)
...
`struct{}` 这是一个空结构体类型。空结构体不占用任何内存空间（大小为0字节），通常用于不需要存储数据但需要结构体类型的场景,比如给通道发信号。
...
io.Writer类型定义了函数Fprintf和这个函数调用者之间的约定。一方面这个约定需要调用者提供具体 类型的值就&#x50CF;_&#x6F;s.File&#x548C;_&#x62;ytes.Buffer，这些类型都有一个特定签名和行为的Write的函数。另一方面这个约定保证了Fprintf接受任何满足io.Writer接口的值都可以工作。
...
![](../../.gitbook/assets/go基础_time_4.png)
...
![](../../.gitbook/assets/go基础_time_5.png)
...
他们的关系如下 ![](../../.gitbook/assets/go基础_time_6.png) 事实上，Copy和CopyN方法都是调用的CopyBuffer方法，不指定大小则会创建一个默认的缓冲区大小为`size := 32 * 1024`
...
简单来说就是，把文件读取进缓冲之后再读取的时候就可以避免文件系统的IO从而提高速度。同理，在进行写操作时，先把文件写入缓冲，然后由缓冲写入文件系统。 ![](../../.gitbook/assets/go基础_time_7.png) bufio.Read(p \[]byte)相当于读取大小len(p)的内容，思路如下：
...
![](../../.gitbook/assets/go基础_time_8.png)
...
一个基于无缓存Channels的发送操作将导致发送者goroutine阻塞，直到另一个goroutine在相同的 Channels上执行接收操作，当发送的值通过Channels成功传输之后，两个goroutine可以继续执行后面的语句。反之，如果接收操作先发生，那么接收者goroutine也将阻塞，直到有另一个goroutine在相同的 Channels上执行发送操作。
...
![](../../.gitbook/assets/go基础_time_9.png)
...
![](../../.gitbook/assets/go基础_time_10.png)

32位系统:

![](../../.gitbook/assets/go基础_time_11.png)

64位系统:

![](../../.gitbook/assets/go基础_time_12.png)
