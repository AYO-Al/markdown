...
Gin使用 [**go-playground/validator/v10**](https://github.com/go-playground/validator) 进行验证。 查看标签用法的全部[文档](https://pkg.go.dev/github.com/go-playground/validator/v10#hdr-Baked_In_Validators_and_Tags). 不满足校验会报错：`Error:Field validation for 'Desc' failed on the 'required' tag`
...
* `Format`：实现时间的格式化，返回字符串。也可以在后端转换在前端使用。
  * \{{.time\_data.Format "2006/01/02 15:04:05"\}}
* `html`：转义文本中的html标签
