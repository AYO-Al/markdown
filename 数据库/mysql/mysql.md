...
| select @@transaction\_isolation; | 查看隔离级别        |                     |
| select @@配置                      | 查看mysql配置     |                     |
| set @@配置                         | 设置mysql配置     |                     |
| 以上命令不区分大小写                       |               |                     |
...
| and              | 并且                                       |                                                        |
| or               | 或者                                       |                                                        |
| in               | 包含，相当于多个or（not in不包含）                    | select \* from car where car\_ID in ('CA001','CA006'); |
| not              | 非                                        |                                                        |
...
* 筛选分组后的数据
  * having关键字可以筛选分组后的数据
  * where和having，优先选择where，where不能完成再选having，比如筛选平均值时，where就不能完成
...
### 8.1.关于分组查询时出现ONLY\_FULL\_GROUP\_BY错误解决方法

提示错误：which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql\_mode=only\_full\_group\_by\
在命令行中执行‘select @@sql\_mode’查看自己的sql\_mode设置
...
该值影响自增长列的插入。默认设置下，插入0或NULL代表生成下一个自增长值。（不信的可以试试，默认的sql\_mode你在自增主键列设置为0，该字段会自动变为最新的自增值，效果和null一样），如果用户希望插入的值为0（不改变），该列又是自增长的，那么这个选项就有用了。
...
1. 在命令行中执行，临时更改：set @@sql\_mode='STRICT\_TRANS\_TABLES,NO\_ZERO\_IN\_DATE,NO\_ZERO\_DATE,ERROR\_FOR\_DIVISION\_BY\_ZERO,NO\_AUTO\_CREATE\_USER,NO\_ENGINE\_SUBSTITUTION';
2.  去my.ini文件中配置，永久更改
...
* 常用存储引擎
  * MyISAM
    * 不支持事物，安全性底![](../../.gitbook/assets/303)
  * InnoDB
    * 表空间存储索引和数据![](../../.gitbook/assets/305)
  * MEMORY![](../../.gitbook/assets/307)

## 15、事务

* 事务命令：
  * 查看隔离级别：select @@transaction\_isolation;
