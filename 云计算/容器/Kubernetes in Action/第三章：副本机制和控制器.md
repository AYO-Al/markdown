...
ReplicationController的工作是保证pod的数量始终与标签选择器的数量相匹配。如果不匹配，ReplicationController则采取适当的操作来协调pod的数量。如下图所示：![](../../../.gitbook/assets/第三章：副本机制和控制器_time_1.png)\
...
> 可以使用`KUBE_EDITOR`环境变量来告诉kubectl使用你期望的文本编辑器，将下面命令放入\~/.bashrc或等效文件中：`export KUBE_EDITOR="/usr/bin/vim"`\
