...
> 本章内容包括：
>
> * Deployment与ReplicaSet控制器的关系
> * 如何实现水平扩缩容
> * 如何实现滚动更新
> * 如何实现版本控制

在上一篇文章中，我为你详细介绍了**Kubernetes项目中第一个重要的设计思想：控制器模式**。
...
而Pod的ownerReference正是这个ReplicaSet。
...
那么，在具体的实现上，这个Deployment，与ReplicaSet，以及Pod的关系是怎样的呢？![](../../../.gitbook/assets/11-作业副本与水平扩展_time_1.png)\
...
nginx-deployment-3167673210   3         3         3       20
```
...
这个随机字符串叫作**pod-template-hash**，在我们这个例子里就是：3167673210。**ReplicaSet会把这个随机字符串加在它所控制的所有Pod的标签里，从而保证这些Pod不会与集群里的其他Pod混淆**。
...
后，在Age=24 s的位置，Deployment Controller开始将这个新的ReplicaSet所控制的Pod副本数从0个变成1个，即：“水平扩展”出一个副本。
...
结合以上讲述，现在我们可以扩展一下Deployment、ReplicaSet和Pod的关系图了。![](../../../.gitbook/assets/11-作业副本与水平扩展_time_2.png)

**如上所示，Deployment的控制器，实际上控制的是ReplicaSet的数目，以及每个ReplicaSet的属性。**

### 1.3 控制应用版本

而**一个应用的版本，对应的正是一个ReplicaSet**；这个版本应用的Pod数量，则由ReplicaSet通过它自己的控制器（ReplicaSet Controller）来保证。

通过这样的多个ReplicaSet对象，Kubernetes项目就实现了对多个“应用版本”的描述。\
而明白了“**应用版本和ReplicaSet一一对应**”的设计思想之后，我就可以为你讲解一下Deployment对应用进行版本控制的具体原理了。

比如在使用`kubectl set image`的指令时，直接修改pod模板中的镜像，且把镜像名字修改为错误的名字。
...
很简单，Deployment对象有一个字段，叫作`spec.revisionHistoryLimit`，就是Kubernetes为Deployment保留的“历史版本”个数。所以，如果把它设置为0，你就再也不能做回滚操作了。

## 2 总结

今天这篇文章中，我为你详细讲解了Deployment这个Kubernetes项目中最基本的编排控制器的实现原理和使用方法。
...
这种场景，光靠Deployment自己就很难应对了。对于这种需求，可以使用“自定义控制器”，就可以帮我们实现一个功能更加强大的Deployment Controller。
